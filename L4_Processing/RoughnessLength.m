...
if sum(good)>20% previosuly 200    
    ...
    x0=[hei*.5 hei*0.1]; %Starting guesses for a and b
    ...
    try
        x=nlinfit(inputs,ubar,@testfun,x0,opts);
        gof=nan;
    catch
        x=x0;
        gof=nan;
        n=nan;
        warning('Bypass Maurer roughness length and went to default values')
    end
...
% so z, USTAR and z_star in our case
    ...
    d=x(1); %Displacement height
    z0=x(2); %Roughness length
    n=sum(good);
    stat=gof;
else
    d=nan;
...
function F =testfun(x0,inputs)
    ...
    F=USTAR./0.4.*(log((z-a)/b)-(2.*log((1+(1-16.*(z-a)./x).^0.25)/2)+log((1+(1-16.*(z-a)./x).^0.5)/2)...
        ...
        -2.*atan((1-16*(b)./x).^0.25)+pi/2)+((1-16.*(z-a)./x).^(-1/4)).*((1+0.5/2.59./((z-a)./(z_star-a)))...
        .*(z-a)./x)./1.5.*log(1+1.5/(2.59.*((z-a)./(z_star-a)))).*exp(-2.59*((z-a)./(z_star-a))));
end