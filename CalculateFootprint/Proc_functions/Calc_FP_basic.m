...
if plotYN==1
    figure;
    if ~isnan(PatchMap)
    %pcolor(FX2,FY2(Rl:-1:1,:),PatchMap*100);shading('flat');% This is the same as using flipud, which flips the rows so that the first one is the last one
    pcolor(FX2,FY2,PatchMap*100);shading('flat');
...
% calculate coordinates for countours   
    [FFP3]= CalcPercNK(footKM,FX2,flipud(-FY2),Rl,contourMarks);
