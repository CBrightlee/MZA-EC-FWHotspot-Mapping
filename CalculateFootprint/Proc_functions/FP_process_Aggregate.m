...
COMP(i).footCUM=nan(length(FY),length(FX));% Cumulative footprint values

COMP(i).Fs0tile=zeros(length(FY),length(FX));% Flux map footprint-weighed
COMP(i).Fs1tile=zeros(length(FY),length(FX));
COMP(i).Fs2tile=zeros(length(FY),length(FX));
COMP(i).Fs3tile=zeros(length(FY),length(FX));
COMP(i).Fs4tile=zeros(length(FY),length(FX));
COMP(i).Fs5tile=zeros(length(FY),length(FX));
COMP(i).Fs6tile=zeros(length(FY),length(FX));
COMP(i).Fs7tile=zeros(length(FY),length(FX));
COMP(i).Fs8tile=zeros(length(FY),length(FX));
COMP(i).Zvegtile=zeros(length(FY),length(FX));

COMP(i).Fs0tile_abs=nan(length(FY),length(FX));% Flux map for CH4 absolute values
COMP(i).Fs1tile_abs=nan(length(FY),length(FX));
COMP(i).Fs2tile_abs=nan(length(FY),length(FX));
COMP(i).Fs3tile_abs=nan(length(FY),length(FX));

COMP(i).footCount=zeros(length(FY),length(FX));% number of time a cell within the end countourMark is counted
COMP(i).footCountS0=zeros(length(FY),length(FX));% number of time a cell within the end countourMark is counted for CH4 footprints
...
COMP(i).footWeight=zeros(length(FY),length(FX));
COMP(i).footWeightS0=zeros(length(FY),length(FX));
...
disp('Entering first loop in FP_process_Aggregate')
...
if ~isnan(ustar(i)*Lo(i)*sv(i)*zo(i)*z(i)*d(i)*windir(i)*h(i)*ubar(i))
    ...
    for j=1:3
        ...
        if j==1
            patchsum_H=FPall(j).PatchSum;
        elseif j==2
            patchsum_K=FPall(j).PatchSum;
        elseif j==3
            patchsum_KM=FPall(j).PatchSum;
        end
        catch
        end
        
    %% Create footprint-weighed flux map
