function [FFP,flag_err]=calc_footprint_FFP_2(zm,z0,umean,h,ol,sigmav,ustar,varargin)
...
if nargin>=7 && nargin <=18
    if nargin > 7
        for f = 1:2:length(varargin)
            switch varargin{f}
                ...
                case 'fig'
                    opt_fig = varargin{f+1};
                otherwise
                    if isnumeric(varargin{f})
                        display(['Ignored unrecognised optional input "' num2str(varargin{f}) '"'])
                    else
    ...
    [ind_return,flag_err,wind_dir,r,nx,crop,fig] = checkinput(zm,z0,umean,h,ol,sigmav,ustar, ...
         opt_wind_dir,opt_r,opt_nx,opt_rslayer,opt_crop,opt_fig);
else
...
if zm/ol>-0.05 % Neutral and stable conditions
...
if xstar_ci_param(2)<0
xstar_ci_param = xstar_ci_param(3:end);
end

if xstar_ci_param(3)<0
xstar_ci_param = xstar_ci_param(4:end);
end
...
FFP.x_80=(-c/log(0.8)+d)*zm*(1-zm/h)^-1*(umean*k/ustar);
%--------------------------------------------------------------------
...
%--------------------------------------------------------------------
   if crop==1
       dminy = floor(min(contour_r(1,:)));
       dmaxy = ceil(max(contour_r(1,:)));
       dminx = floor(min(contour_r(2,:)));
       dmaxx = ceil(max(contour_r(2,:)));
       
       indx  = find(y_2d(:,1)>=dminx & y_2d(:,1)<=dmaxx);
       % extend by one row/column
       indx = [min(indx)-1; indx; max(indx)+1];
       indx = indx(indx>0 & indx<=size(y_2d,1));
       x_2d = x_2d(indx,:);
       y_2d = y_2d(indx,:);
       f_2d = f_2d(indx,:);
       
       indy = find(x_2d(1,:)>=dminy & x_2d(1,:)<=dmaxy);
       % extend by one row/column
       indy = [min(indy)-1 indy max(indy)+1];
       indy = indy(indy>0 & indy<=size(x_2d,2));
       x_2d = x_2d(:,indy);
       y_2d = y_2d(:,indy);
       f_2d = f_2d(:,indy);
   end
...
%--------------------------------------------------------------------
   ...
   FFP(1).y        = y;
       ...
       FFP(1).x_2d_unrot = x_2d;
       FFP(1).y_2d_unrot = y_2d;
       FFP(1).f_2d_unrot = f_2d;
...
if fig==1
    figure;
    colormap jet;
    surf(FFP(1).x_2d,FFP(1).y_2d,FFP(1).f_2d);shading flat;view(2);
    pcolor(FFP(1).x_2d,FFP(1).y_2d,FFP(1).f_2d);shading flat
    hold all;
    if ~isnan(rs)
        for i=1:length(rs)
            z = FFP(i).fr.*100.*ones(size(FFP(i).yr));
            plot3(FFP(i).xr,FFP(i).yr,z,'r');
        end
    end 
    xlabel('x [m]'); ylabel('y [m]');
    axis([min(FFP(1).x_2d(:)),max(FFP(1).x_2d(:)), ...
          min(FFP(1).y_2d(:)),max(FFP(1).y_2d(:))]);
    axis image
    set(gca,'TickDir','out');
    colorbar;
end
...
% Function checkinput
%--------------------------------------------------------------------
function [ind_return,flag_err,wind_dir,r,nx,crop,fig] = checkinput(zm, ...
          z0,umean,h,ol,sigmav,ustar,wind_dir,r,nx,rslayer,crop,fig)
...
if zm<=0
    display('zm must be larger than 0 m')
    ind_return = 1;
elseif h<10
    display('h must be larger than 10 m')
    ind_return = 1;
elseif sigmav<0
    display('sig_v must be larger than 0 m/s')
    ind_return = 1;
elseif ustar<0
    display('ustar must be larger than m/s')
    ind_return = 1;
elseif zm>h
...
if ~isnan(z0)
    if z0 < 0
        display('z0 needs to be larger than 0 m')
        ind_return = 1;
...
elseif ~isnan(umean)
    if umean < 0
        display('umean needs to be larger than 0 m/s')
        ind_return = 1;
    end
