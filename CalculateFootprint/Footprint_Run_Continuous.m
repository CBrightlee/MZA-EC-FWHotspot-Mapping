...
addpath('L4_Processing\')
addpath(genpath('CalculateFootprint'))


processAMFdata=1;
if processAMFdata==1
   L4_Process_from_AFcsv
end

% By Camilo Rey-Sanchez. Last modified Jun 2022.
% Before running this file the file FP_opts_continuous.m must be run

% ONLY adjustments necessary in this file are to the MAP area and PATCHMAP!
% These areas will be indicated by CAPS in the header
...
%% Load data (MAY REQUIRE CHANGE TO PREFERRED MAP)
...
if opt.HaveMap==1
load('MWR_CLASS_map.mat')
map_path='./MWR_CLASS_map.mat';
end
    disp('MapCode.m has run')
...
%% load map or Define Grid (MAY REQUIRE CHANGE TO PREFERRED MAP)
...
if opt.HaveMap==1
    load(map_path);
    PatchMap=Class_Map1; % Change this to match the map you want to depict from MapCode.m
    FX=FX_CLASS; %Make sure FX and FY match with the map
    FY=FY_CLASS;
    PatchCode='NA';
    Cmap=[1,1,1];
    fprintf('Assigned map\n');

elseif opt.mscale==0
    
    lgt=F2H*2 ;% Length of one side of the square with the tower in the center
...
z2=Tower_height_day;%   
d = 0.4*Canopy_height;
zo=0.2267*Canopy_height; %FROM MOUSTAFA AND WANG 2020
...
disp('Running Footprint Code. FP_process_Aggregate beginning')

    %% Run Footprint Code (CHANGE DLM WRITE TO MATCH PREFERRED NAME FOR PATCHSUM OUTPUT)
...
COMP(3).patchsum(isnan(COMP(3).patchsum))=-9999;
dlmwrite('./Footprint_Output/Patchsums/Patchsum_NIGHT.csv',COMP(3).patchsum,',')


    disp('Footprint Code Complete')
...
for i=1:3
    ...
    if ~isnan(PatchMap)
        f222=figure(222);
        i222=pcolor(FX,FY,PatchMap(:,:,1));shading flat;
            else
        f222=figure(222);
        i222=pcolor(FX,FY,PatchMap(:,:,1));shading flat;
      end    
    ...
    [~,cA]=contour(FX2,-FY2(1:RT,:),FG(i).percF,opt.FPcontour,'Fill','off','ShowText','off');%shading('interp') 
...
for i=1:3
    ...
    if opt.models(i)==1
    figure
    kml_contour(LONsave,LATsave(1:RT,:),FG(i).percF,... 
...
clear data
    ...
    save([opt.SaveDir FileLabel opt.Sufix '.mat'],'-v7.3')
